...
fn execute_steps(steps: &Vec<Movement>) -> Submarine{
    let mut submarine: Submarine = Submarine { x: 0, y: 0, aim: 0 };
    for step in steps {
        match step {
            Movement::FORWARD(units) => {
                submarine.x += units;
                submarine.y += units * submarine.aim;
            },
            Movement::UP(units) => submarine.aim -= units,
            Movement::DOWN(units) => submarine.aim += units,
            Movement::UNKNOWN => ()
        }
    }
    return submarine;
